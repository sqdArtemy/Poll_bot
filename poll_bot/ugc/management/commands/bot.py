...
def start(update, context):
    ...
    update.message.reply_text(
        text=get_phrase(update, get_item(update, chat_id,
                        'language', Profile), 'greeting'),
    ...
    for item in items_set:
        ...
        if type == Answers and str(get_item(update, chat_id, 'chosen_ques', Profile)) == str(item.of_question):
            ...
            keyboard.append([InlineKeyboardButton(
                BTNS[item.number], callback_data=text)])
        elif type == Question:
            BTNS[item.number] = text
            keyboard.append([InlineKeyboardButton(
                BTNS[item.number], callback_data=text)])
            ...
            BTNS[item.name] = text
            keyboard.append([InlineKeyboardButton(
                BTNS[item.name], callback_data=text)])
...
def menu(update: Update):
    ...
    if get_item(update, chat_id, 'state', Profile) == 'menu':
        ...
        bot.send_message(
            ...
            reply_markup=ReplyKeyboardMarkup([[KeyboardButton(text=get_phrase(
                update, language(update), 'send_num'), request_contact=True)]], resize_keyboard=True)
        )
...
def message_handler(update: Update, context: CallbackContext):
    ...
    # save user`s name and phone in DB
    if state == 'phone' or state == 'name':
    ...
    if text == wallet:
        phrase = (str(get_phrase(update, language(update), 'balance')) +
                  '  '+str(get_item(update, chat_id, 'wallet', Profile)))
        update.message.reply_text(
            text=phrase
        )
